...
BASE_RECORD = "records/track_10min_16mb/2026-03-20_10L_Int5MLP_MuonWD04_SWA50/train_gpt.py"


def _repo_relative_default(relpath: str) -> str:
    """Resolve defaults from repo root even when launched inside this record folder."""
...
class Hyperparameters:
    ...
    tokenizer_path = os.environ.get("TOKENIZER_PATH", _repo_relative_default("data/tokenizers/fineweb_1024_bpe.model"))
    ...
    use_daffn = _env_bool("USE_DAFFN", "0")
    mlp_schedule = os.environ.get("MLP_SCHEDULE", "uniform")
    use_starrelu = _env_bool("USE_STARRELU", "0")
    ...
    ngram_alpha_base = float(os.environ.get("NGRAM_ALPHA_BASE", 0.10))
    ngram_alpha_max = float(os.environ.get("NGRAM_ALPHA_MAX", 0.50))
    ngram_count_temp = float(os.environ.get("NGRAM_COUNT_TEMP", 10.0))
    ngram_entropy_low = float(os.environ.get("NGRAM_ENTROPY_LOW", 2.0))
    ngram_entropy_high = float(os.environ.get("NGRAM_ENTROPY_HIGH", 5.5))
...
def build_mlp_multipliers(
    ...
    if not use_daffn:
        return [base_mult for _ in range(num_layers)]
    if min_mult <= 0 or max_mult <= 0 or avg_mult <= 0 or min_mult > max_mult:
        raise ValueError(
            f"Invalid DAFFN multipliers: min={min_mult} max={max_mult} avg={avg_mult}"
        )
    if schedule == "uniform":
        raw = [avg_mult for _ in range(num_layers)]
    elif schedule == "linear_increasing":
        raw = np.linspace(min_mult, max_mult, num_layers).tolist()
    elif schedule == "linear_decreasing":
        raw = np.linspace(max_mult, min_mult, num_layers).tolist()
        ...
        if num_layers == 1:
            ...
            raw = [
                min_mult + (max_mult - min_mult) * (1.0 - min(abs(i - center) / denom, 1.0))
                for i in range(num_layers)
            ]
    elif schedule == "alternating":
        raw = [max_mult if i % 2 else min_mult for i in range(num_layers)]
    else:
        raise ValueError(
            "MLP_SCHEDULE must be one of: uniform, linear_increasing, "
            "linear_decreasing, middle_heavy, alternating"
        )

    raw_mean = sum(raw) / len(raw)
    scaled = [m * (avg_mult / raw_mean) for m in raw]
    return [min(max(m, min_mult), max_mult) for m in scaled]


class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, use_starrelu: bool = False, hidden: int | None = None):
        super().__init__()
        hidden = int(hidden if hidden is not None else mlp_mult * dim)
        self.hidden = hidden
        self.mlp_mult = float(hidden) / float(dim)
        self.use_starrelu = use_starrelu
        self.fc = CastedLinear(dim, hidden, bias=False)
        self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj._zero_init = True
        if use_starrelu:
            self.starrelu_scale = nn.Parameter(torch.ones((), dtype=torch.float32))
            self.starrelu_bias = nn.Parameter(torch.zeros((), dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        x = torch.relu(self.fc(x))
        x = x.square()
        if self.use_starrelu:
            x = self.starrelu_scale.to(dtype=x.dtype) * x + self.starrelu_bias.to(dtype=x.dtype)
        return self.proj(x)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, use_starrelu=use_starrelu, hidden=mlp_hidden)
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        use_daffn: bool = False,
        mlp_schedule: str = "uniform",
        use_starrelu: bool = False,
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    num_heads,
                    num_kv_heads,
                    self.mlp_multipliers[layer_idx],
                    rope_base,
                    qk_gain_init,
                    use_starrelu=use_starrelu,
                    mlp_hidden=self.mlp_hidden_sizes[layer_idx],
                )
...
def count_parameters_by_component(model: nn.Module) -> dict[str, int]:
    ...
    for name, param in model.named_parameters():
        ...
        if "tok_emb" in name or "lm_head" in name or "bigram" in name:
            ...
            counts["normalization_scalars_other"] += n
    return counts


def format_mlp_schedule(model: GPT) -> str:
    parts = [
        f"L{i}:hidden={hidden}:mult={mult:.3f}"
        for i, (hidden, mult) in enumerate(zip(model.mlp_hidden_sizes, model.mlp_multipliers, strict=True))
    ]
    return " ".join(parts)
...
def maybe_log_ngram_mix_status(args: Hyperparameters, log0) -> None:
    ...
    # scoring. This record keeps the scaffold off by default and does not alter BPB.
    log0(
        f"ngram_mix:enabled:{args.use_ngram_mix} orders:{args.ngram_min_order}-{args.ngram_max_order} "
        f"alpha_base:{args.ngram_alpha_base} alpha_max:{args.ngram_alpha_max} "
        f"count_temp:{args.ngram_count_temp} entropy_low:{args.ngram_entropy_low} "
        f"entropy_high:{args.ngram_entropy_high}"
    )
    if args.use_ngram_mix:
        log0(
            "ngram_mix:scaffold_only neural_only_scoring:true table_sizes:{} memory_estimate_bytes:0 "
            "avg_alpha:0.000 alpha_entropy_buckets:{} "
            "reason:disabled_until_training-only_table_build_and_artifact_accounting_are_verified"
        )
...
def main() -> None:
    ...
    args = Hyperparameters()
    if _env_bool("MODEL_INIT_CHECK", "0"):
        check_model = GPT(
            ...
            use_daffn=args.use_daffn,
            mlp_schedule=args.mlp_schedule,
            use_starrelu=args.use_starrelu,
    ...
    log0(
        "base_techniques:SP1024 tokenizer, 10L GPT, GQA, QK gain, SmearGate, BigramHash(10240), "
        "U-Net skips, relu^2 MLP3x, Muon WD, SWA, mixed int5/int6 compression, zstd/zlib, sliding eval"
    )
    log0(f"tokenizer_kind:sentencepiece tokenizer_path:{args.tokenizer_path} vocab_size:{args.vocab_size}")
    log0(
        f"architecture:gpt_unet num_layers:{args.num_layers} model_dim:{args.model_dim} "
    ...
    log0(
        ...
        f"{args.muon_momentum_warmup_steps} AdamW scalar_lr:{args.scalar_lr} token_lr:{token_lr} "
        f"weight_decay:{args.weight_decay}"
    )
    log0(f"ema_enabled:false swa_enabled:{args.swa_enabled} swa_start_frac:{args.swa_start_frac} swa_every:{args.swa_every}")
    log0(f"quantization:mixed_int5_mlp_int6_attention_fp16_sensitive_tensors compressor:{_COMPRESSOR}")
    log0(f"expected_artifact_limit_bytes:{ARTIFACT_LIMIT_BYTES}")
    log0(f"param_components:{component_counts}")
    maybe_log_ngram_mix_status(args, log0)
    ...
    while True:
        ...
        if should_validate:
            ...
            tokens_sec = 0.0 if step == 0 else (args.train_batch_tokens * step) / max(training_time_ms / 1000.0, 1e-9)
            log0(
                f"step:{step}/{args.iterations} val_loss:{val_loss:.4f} val_bpb:{val_bpb:.4f} "
                f"train_time:{training_time_ms:.0f}ms step_time:{step_avg_ms:.2f}ms "
                f"step_avg:{step_avg_ms:.2f}ms tokens_sec:{tokens_sec:.0f}"
            )
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        ...
        if should_log_train:
            ...
            train_tokens_sec = (args.train_batch_tokens * step) / max(approx_training_time_ms / 1000.0, 1e-9)
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_time:{train_step_avg_ms:.2f}ms "
                f"step_avg:{train_step_avg_ms:.2f}ms tokens_sec:{train_tokens_sec:.0f}"
            )
    ...
    if master_process:
        ...
        artifact_status = "PASS" if total_artifact_bytes <= ARTIFACT_LIMIT_BYTES else "FAIL"
        log0(f"Serialized model int6+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"compressed_model_size_bytes:{quant_file_bytes}")
        log0(f"total_artifact_size_estimate_bytes:{total_artifact_bytes}")
        log0(f"artifact_limit_check:{artifact_status} limit_bytes:{ARTIFACT_LIMIT_BYTES} total_bytes:{total_artifact_bytes}")
        log0(f"Total submission size int8+zlib: {total_artifact_bytes} bytes")
    ...
    if args.use_ngram_mix:
        log0(f"ngram_mix:val_bpb_neural_only:{q_val_bpb:.8f} val_bpb_neural_plus_ngram:not_computed")
