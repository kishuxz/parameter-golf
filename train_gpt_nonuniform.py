...
def get_layer_hidden_sizes(
    ...
    for i in range(num_layers):
        if num_layers == 1:
            ...
            t = i / (num_layers - 1)
        mult = mlp_mult_min + t * (mlp_mult_max - mlp_mult_min)
        hidden = int(round(dim * mult / 64) * 64)
        sizes.append(hidden)
    return sizes
...
class StarReLU(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return self.scale * F.relu(x).square() + self.bias
...
class GPT(nn.Module):
    def __init__(
        ...
        hidden_sizes = get_layer_hidden_sizes(num_layers, model_dim, mlp_mult_min, mlp_mult_max)
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, num_kv_heads, hidden_sizes[i], rope_base, qk_gain_init)
                for i in range(num_layers)
            ]
        )
